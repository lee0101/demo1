...
export default {
    ...
    methods: {
        ...
        scrollToCurrent(current) {//播放的歌曲  在第一位   滚动到当前播放的歌曲
            const index = this.sequenceList.findIndex((song) => {
                return current.id === song.id
            })
            this.$refs.listContent.scrollToElement(this.$refs.list.$el.children[index], 300)//第一个值是 dom 元素， 第二个是动画时间，表示滑动到某个元素
        },
        deleteOne(item) {
 if (item.deleting) {//删除歌曲 主要逻辑放在action里面
          return
        }
item.deleting = true
