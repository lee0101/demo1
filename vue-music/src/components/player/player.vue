...
export default {
    ...
    methods: {
        ...
        updateTime(e) { //audio 返回的播放时间
            this.currentTime = e.target.currentTime
        },
        format(interval) {
            interval = interval | 0 //或0 标签 --  向下取整数
        ...
        getLyric() { //获取歌词
            this.currentSong.getLyric().then((lyric) => {
                ...
                if (this.isPureMusic) {
                    ...
                    if (this.playing && this.songReady) {//歌曲已经准备好  并且可以播放了
                        //這时候有可能 用户已经播放了歌曲，要切换到对应位置
                        //const currentTime = this.currentSong.duration*this.percent*1000
                        this.currentLyric.seek(this.currentTime * 1000)
                    }
                }
        ...
        handleLyric({ lineNum, txt }) { //回调
            ...
            this.playingLyric = txt //显示当前播放的歌词ss
        },
        showPlaylist() {
